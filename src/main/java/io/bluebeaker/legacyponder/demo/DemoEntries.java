...
import io.bluebeaker.legacyponder.LegacyPonderConfig;
...
public class DemoEntries {
    ...
    public static void addDemoIfNeeded(){
        ...
        } else if (!LegacyPonderConfig.demo && demoAdded) {
    ...
    public static boolean addDemoEntry(){
        ...
        entry.addPage(PonderPageBase.fromDrawable(new IDrawableSupplier() {
            @Override
        ...
        entry.addPage(PonderPageBase.fromStructure("legacyponder:tree").setDescription("A tree, with some blocks around it."));
