...
public class UnconfusionRecipeCategory implements IRecipeCategory<UnconfusionRecipeWrapper> {
    public static final int recipeWidth = 160;
    public static final int recipeHeight = 125;
    public static final String UID = LegacyPonder.MODID+".unconfusion";
    private final IDrawable background;
    private final IDrawable icon;
    private final IDrawable slotBackground;
    private final FluidStackRenderer fluidStackRenderer = new FluidStackRenderer();

    public UnconfusionRecipeCategory(IGuiHelper guiHelper) {
...
//        this.icon = guiHelper.createDrawableIngredient(new ItemStack(Blocks.BOOKSHELF));
    ...
    @Override
    public void setRecipe(IRecipeLayout recipeLayout, UnconfusionRecipeWrapper unconfusionRecipeWrapper, IIngredients ingredients) {
        int itemCount = unconfusionRecipeWrapper.getItems().size();
        int fluidCount = unconfusionRecipeWrapper.getFluids().size();
