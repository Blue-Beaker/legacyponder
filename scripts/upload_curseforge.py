...
def find_jar_file(mod_name: str, version: str, dev_suffix: bool = False) -> Path:
    """Find the built jar file for the given version."""
    if dev_suffix:
        jar_name = f"{mod_name}-{version}-dev.jar"
    else:
        jar_name = f"{mod_name}-{version}.jar"
    ...
    pattern = re.compile(rf"{re.escape(mod_name)}-{re.escape(version)}.*\.jar$")
...
def main():
    ...
    if args.file:
        ...
        jar_path = find_jar_file(mod_name, mod_version, dev_suffix=args.dev)
        if not jar_path:
            print(f"Error: Could not find jar for {mod_name}-{mod_version} in {BUILD_LIBS_DIR}")
